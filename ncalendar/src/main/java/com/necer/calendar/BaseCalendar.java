...
public abstract class BaseCalendar extends ViewPager implements ICalendar {
    ...
    protected List<LocalDate> mDonotuseList;
    ...
    private void callBack() {
        post(new Runnable() {
            ...
            public void run() {
                CalendarView currectCalendarView = findViewWithTag(getCurrentItem());
                LocalDate middleLocalDate = null;
                List<LocalDate> currentSelectDateList = new ArrayList<>();
                try {
                    middleLocalDate = currectCalendarView.getMiddleLocalDate();
                    currentSelectDateList = currectCalendarView.getCurrentSelectDateList();
    ...
    private void clickDisableDate(LocalDate localDate) {
        ...
        if (mOnClickDisableDateListener != null) {
            ...
            Toast.makeText(getContext(), TextUtils.isEmpty(mAttrs.disabledString) ? "日期不可选" : mAttrs.disabledString, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void jumpMonth(int year, int month) {
        LocalDate jumpDate;
        try {
            jumpDate = new LocalDate(year, month, 20);
        } catch (Exception e) {
    ...
    @Override
    public void setDonotuseList(List<String> donotuserList) {
        mDonotuseList.clear();

        for (int i = 0; i < donotuserList.size(); i++) {
            LocalDate holidayLocalDate;
            try {
                holidayLocalDate = new LocalDate(donotuserList.get(i));
            } catch (Exception e) {
                throw new RuntimeException("setLegalHolidayList集合中的参数需要 yyyy-MM-dd 格式的日期");
            }
            mDonotuseList.add(holidayLocalDate);
        }
    ...
    public boolean isAvailable(LocalDate localDate) {
        return !localDate.isBefore(mStartDate) && !localDate.isAfter(mEndDate) && !mDonotuseList.contains(localDate);
    }

    public List<LocalDate> getmDonotuseList() {
        return mDonotuseList;
    }
