...
public class MNCalendarVerticalItemAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof MyViewHolder) {
            ...
            boolean flag = false;
//            判断日期是否需要标记
            for (int i = 0; mnCalendarVerticalConfig.getMnCalendar_dateList()!= null && i < mnCalendarVerticalConfig.getMnCalendar_dateList().size(); i++) {
                if(times(datePosition.getTime()).equals(mnCalendarVerticalConfig.getMnCalendar_dateList().get(i)))
                {
                    myViewHolder.datePoint.setVisibility(View.VISIBLE);
                    flag = true;
                    break;
                }
            }
            ...
            if(!flag)
            {
                myViewHolder.datePoint.setVisibility(View.GONE);
            }
            ...
            if (adapter.endDate != null && adapter.endDate == datePosition) {
                ...
                myViewHolder.tv_small.setVisibility(View.VISIBLE);
                if(adapter.startDate == datePosition)
                {
                    myViewHolder.tv_small.setText("开始-结束");
                    myViewHolder.iv_bg.setBackgroundResource(R.drawable.mn_selected_bg);
                }
                else
                {
...
//        int i = Integer.parseInt(time);
    ...
    private static class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(View itemView) {
            ...
            datePoint =  (View) itemView.findViewById(R.id.datePoint);
        }
