...
public class MessageProjectCoordinationActivity extends MvpActivity<MessageProjectCoordinationView, MessageProjectCoordinationPresenter> implements MessageProjectCoordinationView {
    ...
    public class MemberInfoApter extends BaseQuickAdapter<MessageProjectCoordinationViewModel.DataBeanX.DataEntity, BaseViewHolder> {
        ...
        protected void convert(BaseViewHolder baseViewHolder, MessageProjectCoordinationViewModel.DataBeanX.DataEntity dataBean) {
            ...
            baseViewHolder.setText(R.id.time, dataBean.getExecution_time());
            ...
            for (int i = 0; i < dataBean.getTags().size(); i++) {
                if (i == 0) {
                    baseViewHolder.setText(R.id.tv_label1, dataBean.getTags().get(i));
                    baseViewHolder.setGone(R.id.tv_label1, true);
                } else if (i == 1) {
                    baseViewHolder.setText(R.id.tv_label2, dataBean.getTags().get(i));
                    baseViewHolder.setGone(R.id.tv_label2, true);
                } else if (i == 2) {
                    baseViewHolder.setText(R.id.tv_label3, dataBean.getTags().get(i));
...
//            状态 0- 申请中 1-已同意 2-已拒绝 3-已过期
            ...
            baseViewHolder.getView(R.id.header).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Bundle bundle = new Bundle();
                    bundle.putString("uid", dataBean.getUser_id());
                    Tools.jump(MessageProjectCoordinationActivity.this, HomeageActivity.class, bundle, false);
                }
            });
        }
    }
