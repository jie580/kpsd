 ...
 * 个人中心-订单-商品
 */
public class OrderCompanyGoods extends BaseFragment {
    ...
    public static OrderCompanyGoods newInstance() {
        return new OrderCompanyGoods();
    }
    ...
    private void getList()
    {
        ...
        new HttpUtil().sendRequest(Constant.ORDER_GOODS_ORDER, param, new CommonCallback() {
            @Override
            public void onSuccessJson(String data, int code) {
                dataList = new Gson().fromJson(data, ProjectGoodsListBean.class);

                listView.setLayoutManager( new LinearLayoutManager(getContext()));
                listAdapter = new OrderCompanyGoodsListAdapter(dataList.getData().getData(),getActivity());
...
//                                    回仓
                            ...
                            case R.id.moneyBtn:
                                ...
                                dialogProjectSelect.setItemType("goods");
                                if(dataBean.getStatus().equals("0") && Integer.parseInt(dataBean.getCountDown()) > 0)
                                {
//                                    拒绝加入
